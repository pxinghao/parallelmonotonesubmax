...
\begin{algorithmic}
  ...
  \IF{$dep_e \cap (S^i \backslash S^{i_e}) = \emptyset$}
...
Instead of explicitly constructing dependencies, \pcclz{} computes a lower bound $\widetilde\rho_e$ on the marginal gain in addition to the upper bound $\rho_e$.
To make the decision to add an element $e$ to $S$, it suffices to have $\widetilde\rho_e \geq \rho_{e'}$ for all other elements $e' \not\in S$.

\xinghao{The \pcclz{} approach may turn out to be less efficient than \occlz{} because of the need to compute $\widetilde\rho_e$.
...
Here, we define ``progress'' as either computing tight bounds $\rho_e = \widetilde\rho_e$, or advancing $i_e$.}
...
\begin{algorithmic}[1]
  ...
  \FOR{$p = 1, \dots, P$}
    ...
    \IF{$i_e = i$}
      \STATE $\rho_e \leftarrow FAIL$, $\widetilde\rho_e \leftarrow FAIL$
    \ELSE
      \STATE $\rho_e     \leftarrow F(S_e\cup e) - F(S_e)$
      \STATE $\widetilde\rho_e \leftarrow F(\widehat{S}_e\cup e) - F(\widehat{S}_e)$
    \ENDIF
    \STATE $commit(e, \rho_e, \widetilde\rho_e, t_e, i_e)$
  \ENDFOR
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[tb]
  \caption{$commit(e, \rho_e, \widetilde\rho_e, t_e, i_e)$}
...
\begin{algorithmic}
  ...
  \IF{$\rho_e = FAIL$}
    ...
    \STATE $\widetilde\rho_e \leftarrow \rho_e$
  \ENDIF
  \STATE $lock(Q)$
